...
- **Assistant access** uses Google sign-in (NextAuth/Auth.js) restricted to the owner's email plus an admin-editable allow-list in Site Settings. Anything an assistant submits is created as a Sanity draft (`drafts.` id) — never published directly, exactly as the brief specifies. Mo's own posts from `/post` publish immediately, since she's already previewed them in the app before hitting Publish.
